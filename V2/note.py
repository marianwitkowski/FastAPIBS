...
def create_note(payload: schemas.NoteSchema, db: Session = Depends(get_db)):
...
@router.get("/{noteId}")
def get_post(noteId: int, db: Session = Depends(get_db)):
...
def delete_post(noteId: int, db: Session = Depends(get_db)):
